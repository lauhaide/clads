...
def run(args):
    ...
    TO_SHORTEN_LANGS = args.toShortenLangs.split('-')  # e.g., should give ['de', 'en']
    assert len(TO_SHORTEN_LANGS) == 2
        ...
        if smallerTitle in smallerTitleList:
            if not smallerTitle in titlesMap0.keys():
                ...
                if toShortenTitle0 in smallerTitleList0 \
                    ...
                    failed0 = TO_SHORTEN_LANGS[0] if toShortenTitle0 in smallerTitleList0 else ""
                    failed1 = TO_SHORTEN_LANGS[1] if toShortenTitle1 in smallerTitleList1 else ""
                    smallerTitleSubsetReduced.write(smallerTitle + "\t" + "|".join([failed0, failed1]) + "\n")
    ...
    print("done")
    print("Len of reduced titles: ", len(titlesMap0.values()), len(titlesMap1.values()))
