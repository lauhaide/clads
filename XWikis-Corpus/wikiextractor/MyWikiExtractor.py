...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        #if not self.id in curids:
        #    logging.info('%sSkip \t%s', self.id, self.title)
        #    return
        ...
        urlsFile.write(" ||| ".join([self.title, get_url(self.id)]) + "\n")
        urlsFile.flush()
...
def main():
    ...
    groupP.add_argument("--outFileName", type=str, default='', required=True,
                        help="Name of the file with title - url lines to be generated.")
    ...
    global urls, urlsFile
    urls = []
    urlsFile = open(os.path.join(args.home, args.xwikisDir, args.outFileName), 'w')
