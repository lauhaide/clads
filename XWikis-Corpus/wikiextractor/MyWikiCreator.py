...
options = SimpleNamespace(
    ...
    keepSections = True,
    keepLevel = False, #Whether to print the level of the section too when output is plain text.
...
str_len_constraints = "{}-{}.{}-{}-per"\
    .format(MIN_LEAD_LEN, MAX_LEAD_LEN, MIN_DOC_LEN, MAX_DOC_LEN) \
    if lenConstraints else "none"
...
class Extractor(object):
    ...
    #    ##write here the way we want to select the articles e.g. those with curid in a category set
    #    return True
    ...
    @staticmethod
    def flatText(document):
        str = []
        for h2 in document:
            str.append("<h{}> {} </h{}>".format(2, h2['title'], 2))
            str.append(h2['text'])
            if h2['children']:
                for h3 in h2['children']:
                    str.append("<h{}> {} </h{}>".format(3, h3['title'], 3))
                    str.append(h3['text'])
                    if h3['children']:
                        for h4 in h3['children']:
                            str.append("<h{}> {} </h{}>".format(4, h4['title'], 4))
                            str.append(h4['text'])
        return " ".join(str)

    @staticmethod
    def shortenLead(lead, MAX_LEAD_LEN):
        counts = 0
        tmp = []
        for p in lead.split("<p>"):
            toAdd = len(p.split())
            if counts + toAdd < MAX_LEAD_LEN:
                tmp.append(p)
                counts += toAdd
            else:
                break
    ...
    #    paras = lead.split("<p>")
    #    if len(paras) == 1:
    #        para_toks = paras[0].split()[:MAX_LEAD_LEN]
    #        return " ".join(para_toks)
    #    else:
    #        counts = 0
    #        tmp = []
    #        for p in lead.split("<p>"):
    ...
    @staticmethod
    def structureDocument(document, level, PERCEN_SHORTEN):
        """I hand-traced this piece of code, seems to work well!"""

        def cropWhatIsPossible(text, children):
            if len(text.split()) > 400 and children:
                # section is large enough, leave out children
                children = []
            elif children:
                # section is small, reduce children's text
                newChildren = []
                for c in children:
                    tmp = c['text'].split()
                    if len(tmp) > 200:
                        take = int(PERCEN_SHORTEN * len(tmp))
                        c['text'] = " ".join(tmp[0:take])
                    newChildren.append(c)
                children = newChildren
            else:
                tmp = text.split()
                take = int(PERCEN_SHORTEN * len(tmp))
                text = " ".join(tmp[0:take])
        ...
        for line in document:
            if line.startswith("<h{}".format(level)):
                seeSection = True
                # consume up to next level item
                if levelItemContent:
                    text, children = Extractor.structureDocument(levelItemContent, level+1, PERCEN_SHORTEN)
                    # try to shorten lower levels
                    text, children = cropWhatIsPossible(text, children)

                    siblings.append({'level': level,
                                 'title': sectionTitle,
                                 'text': text,
                                 'children': children })
                    levelItemContent = []

                sectionTitle = line[4:].split('</')[0]
            elif not seeSection:
                bodyText.append(line)
            else:
                levelItemContent.append(line)

        # process last sibling
        if levelItemContent:
            text, children = Extractor.structureDocument(levelItemContent, level + 1, PERCEN_SHORTEN)
            # try to shorten lower levels
            if level == 3:
    ...
    def extract(self, out):
        ...
        if not self.title.lower() in titles:
            return
        logging.info('%s\t%s', self.id, self.title)

        if self.title.lower() in titlesDone:
            return
        
        # Separate header from text with a newline.
        if options.toHTML or options.keepLevel:
        ...
        # overall length constraint in the Wikipedia article
        if (sum(len(line) for line in text) + 1) < options.min_text_length: # add 1 in sum cause the title is added below now
            ruledoutFile.write("{} ||| {} ||| options.min_text_length constraint\n".format(self.id, self.title))
            ruledoutFile.flush()
            return

        # separate lead from rest of the document
        lead, document = [], []
        docStart = False
        prevLine = ''
        for line in text:
            line = line.strip()
            if line =='<html>' or line=='<h2></h2>' or line=='<h3></h3>' or line=='<h4></h4>':
                continue
            if line.startswith('<h'):
                docStart = True
            line = '<p>' if line == '' else line
            if line.startswith('<h'):
                if len(line[5:].split('</'))==1:
                    ruledoutFile.write("{} ||| {} ||| Ill formed\n".format(self.id, self.title));
                    ruledoutFile.flush()
                    return
                section_title = line[4:].split('</')[0]
                line = line[0:4] + " " + section_title + " " +  '</'+line[5:].split('</')[1]
                if section_title.lower() in stopSections:
                    # reach the bottom of the document, stop here
                    break
            if not (prevLine.startswith('<p>') and  line == '<p>'): # eliminate consecutive line breaks
                if not docStart:
                    lead.append(line)
                else:
        ...
        if len(lead) < 1:  ## this should not happen but just print in case for debug
            logging.warn('Empty lead %s\t%s', self.id, self.title)
            ruledoutFile.write("{} ||| {} ||| Empty lead\n".format(self.id, self.title)); ruledoutFile.flush()
            return

        lead = lead[:-1] if lead[-1] == '<p>' else lead #remove tailing <p>s .

        if len(document) < 1: ## this should not happen but just print in case for debug
            logging.warn('Empty document %s\t%s', self.id, self.title)
            ruledoutFile.write("{} ||| {} ||| Empty document\n".format(self.id, self.title)); ruledoutFile.flush()
            return

        if len(document) == 1: logging.warn('Document one line %s\t%s', self.id, self.title)

        document = document[:-1] if document[-1] == '<p>' else document

        # check length constraints, if not skip the article
        lead = " ".join(lead)
        leadlen = len(lead.split())
        if lenConstraints and (leadlen < MIN_LEAD_LEN):
            ruledoutFile.write("{} ||| {} ||| Lead out of min len\n".format(self.id, self.title)); ruledoutFile.flush()
            return
        if lenConstraints and leadlen > MAX_LEAD_LEN:
            lead = Extractor.shortenLead(lead, MAX_LEAD_LEN)

            # 17/10/2020 following if
            if len(lead.split()) < MIN_LEAD_LEN:  ## this should not happen but just print in case for debug
                logging.warn('Issues when shortenning lead %s\t%s', self.id, self.title)
                ruledoutFile.write("{} ||| {} ||| Issues when shortenning lead\n".format(self.id, self.title));
                ruledoutFile.flush()
                return

        lendoc = len(" ".join(document).split())
        if lenConstraints and (lendoc < MIN_DOC_LEN):
            ruledoutFile.write("{} ||| {} ||| Body out of min len\n".format(self.id, self.title)); ruledoutFile.flush()
            return
        if lenConstraints and lendoc > MAX_DOC_LEN:
            exeed = lendoc - MAX_LEAD_LEN
            if exeed in range(0, 500): PERCEN_SHORTEN = 0.9
            if exeed in range(500,1000) : PERCEN_SHORTEN = 0.8
            if exeed in range(1000,2000): PERCEN_SHORTEN = 0.6
            if exeed >3000: PERCEN_SHORTEN = 0.4
            _, document = Extractor.structureDocument(["start"] + document, 2, PERCEN_SHORTEN)
            document = Extractor.flatText(document)
            if len(document.split()) > MAX_DOC_LEN:
                ruledoutFile.write("{} ||| {} ||| Body out of max len (fail to shorten)\n".format(self.id, self.title))
                ruledoutFile.flush()
                return
            if len(document.split()) < MIN_DOC_LEN:
                ruledoutFile.write("{} ||| {} ||| Issues when shortenning body \n".format(self.id, self.title))
                ruledoutFile.flush()
                return
        else:
            document = " ".join(document)

        document = document.replace('<p> <h', '<h')

        text = [title_str] + text
        #self.write_output(out, text)

        outTitles.write( self.title + "\n"); outTitles.flush()
        outSum.write(lead + "\n"); outSum.flush()
        outDoc.write(document + "\n"); outDoc.flush()
            ...
            if line:  # FIXME: n is '"'
                if options.keepLists:
                    if options.keepSections:
                        if options.keepLevel:
                            # emit open sections preserving level information
                            items = sorted(headers.items())
                            for l, v in items:
                                page.append("<h%d>%s</h%d>" % (l, v, l))
                        else:
                            # emit open sections
                            items = sorted(headers.items())
                            for _, v in items:
    ...
    for page_data in pages_from(input):
        ...
        if keepPage(ns, page):
            ...
            if spool_length.value > max_spool_length:
                #print("***************************************************************************** {}".format(spool_length.value) + title )
    ...
    while True:
        ...
        if job:
            ...
            try:
                ...
                ruledoutFile.write("{} ||| {} ||| Error processing\n".format(id, title))
                ruledoutFile.flush()
...
def main():
    ...
    groupP.add_argument("-e", "--level", action="store_true",
                        help="preserve sections's level information")
    ...
    # special for XWikis
    groupP.add_argument("--lang", type=str, choices=['en', 'de', 'fr', 'cs', 'zh'],
                        help="Language of the current wiki extraction")
    groupP.add_argument("--home", type=str, default='', required=True,
                        help="Home where all XWikis related files are.")
    groupP.add_argument("--xwikisDir", type=str, default='xwikis-raw', required=True,
                        help="Directory name where all XWikis related files are.")
    groupP.add_argument("--pivotName", type=str, default='', required=True,
                        help="The language links file with titles across languages.")
    groupP.add_argument("--pivotLang", type=str, default='', required=True,
                        help="The privot language of the language links file.")
    groupP.add_argument("--outDirPrefix", type=str, default='', required=True,
                        help="Prefix string to create output directory name in "
                             "concatenation with used length constraints (e.g., 'de-fr-en').")
    ...
    HOME = args.home
    global pivotFile, outDoc, outSum, outTitles, ruledoutFile, titles, titlesDone, stopSections, stopSectionsFile

    pivotName = os.path.join(HOME, args.xwikisDir, args.pivotName)
    pivotLang = args.pivotLang
    outdir = args.outDirPrefix + '.' + str_len_constraints

    print(" * Extracting Wikipedia in: {}".format(options.lang))
    pivotFile = open(pivotName, 'r')

    # build the list of titles for the corresponding language
    titles = []
    for line in pivotFile.readlines():
        l = line.strip().split(" ||| ")
        if options.lang == pivotLang:
            titles.append(l[0].strip().lower())
        else:
            for lt in l:
                if lt.split('::')[0] == options.lang:
                    titles.append(lt.split('::')[1].strip().lower())

    print(" * Titles for extraction: {}".format(len(titles)))

    doneFileName = os.path.join(args.home, args.xwikisDir, outdir, '{}_titles_done.txt'.format(options.lang))
    titlesDone = []
    if os.path.isfile(doneFileName):
        for l in open(doneFileName,'r'):
            titlesDone.append(l.strip().lower())
    print(" * Titles done: {}".format(len(titlesDone)))

    stopSectionsFile = open(os.path.join(HOME, args.xwikisDir, '{}_stop_sections.txt'.format(options.lang)), 'r')
    stopSections = [s.strip().lower() for s in stopSectionsFile.readlines()]

    # Will generate these output files containing the titles that meet the
    # document/summary lengths constraints.
    outDoc = open(os.path.join(HOME, args.xwikisDir, '{}/{}_documents.txt'.format(outdir, options.lang)), 'w')
    outSum = open(os.path.join(HOME, args.xwikisDir, '{}/{}_summaries.txt'.format(outdir, options.lang)), 'w')
    outTitles = open(os.path.join(HOME, args.xwikisDir, '{}/{}_titles.txt'.format(outdir, options.lang)), 'w')
    ruledoutFile = open(os.path.join(HOME, args.xwikisDir, '{}/{}_titles_outoflen.txt'.format(outdir, options.lang)), 'w')
    ...
    outTitles.close()
    ruledoutFile.close()
    print(" * Titles to extract for {}: {}".format(options.lang, len(titles)))
    print(" * Length constraints: " + str_len_constraints)
    print(" * Pivot file: " + pivotName)
