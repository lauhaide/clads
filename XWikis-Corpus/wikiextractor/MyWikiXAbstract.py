...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        if (sum(len(line) for line in
                text) + 1) < options.min_text_length:  # add 1 in sum cause the title is added below now
...
def main():
    ...
    # special for XWikis
    groupP.add_argument("--srcLang", type=str, choices=['en', 'de', 'fr', 'cs', 'zh'],
                        help="Language of the source documents it pairs with.")
    groupP.add_argument("--lang", type=str, choices=['en', 'de', 'fr', 'cs', 'zh'],
                        help="Language of the current wiki abstract extraction")
    ...
    HOME = args.home
    global pivotFile, outSum, outTitles, ruledoutFile, titles, titlesWithDocuments, srcLang, tgtLang, stopSections, stopSectionsFile
    ...
    # will do extraction of abtracts for these titles
    titlesToGoodSize = os.path.join(HOME, args.xwikisDir, outdir, '{}_titles.txt'.format(srcLang))
    titlesWithDocuments = []
    if os.path.isfile(titlesToGoodSize):
        for l in open(titlesToGoodSize, 'r'):
            titlesWithDocuments.append(l.strip().lower())

    print(" * Extracting Wikipedia in: {}->{} for {} src documents".format(srcLang, tgtLang, len(titlesWithDocuments)))
    print(" * Length constraints: " + str_len_constraints)
    print(" * Pivot file: " + pivotName)

    pivotFile = open(pivotName, 'r')
    ll = [1 for t in pivotFile.readlines()]
    pivotFile.seek(0)
    pbar = tqdm(total=len(ll))

    # build the list of titles for the corresponding language
    titles = []
    for nb, line in enumerate(pivotFile.readlines()):
        l = line.strip().split(" ||| ")
        if tgtLang == pivotLang:
            ttgt = l[0].strip().lower()
        if srcLang == pivotLang:
            tsrc = l[0].strip().lower()

        for lt in l:
            if lt.split('::')[0] == tgtLang:
                ttgt = lt.split('::')[1].strip().lower()
            if lt.split('::')[0] == srcLang:
                tsrc = lt.split('::')[1].strip().lower()
        if tsrc in titlesWithDocuments and ttgt not in titles:
            titles.append(ttgt)

        if (nb % 200) == 0:
            pbar.update(200)

    print(" * Titles with documents for abstract extraction: {}".format(len(titles)))

    stopSectionsFile = open(os.path.join(HOME, args.xwikisDir, '{}_stop_sections.txt'.format(tgtLang)), 'r')
    ...
    outSum = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_{}_summaries.txt'.format(srcLang, tgtLang)), 'w')
    outTitles = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_{}_titles.txt'.format(srcLang, tgtLang)), 'w')

    ruledoutFile = open(os.path.join(HOME, args.xwikisDir, outdir,
                                     '{}_{}_titles_outoflen.txt'.format(srcLang, tgtLang)), 'w')
    ...
    outTitles.close()
    ruledoutFile.close()
    print(" * Extracting Wikipedia in: {}->{}".format(srcLang, tgtLang))
    print(" * Abstracts to extract for {} documents: {}".format(srcLang, len(titles)))
    print(" * Length constraints: " + str_len_constraints)
    print(" * Pivot file: " + pivotName)
