...
def main():
    ...
    HOME = args.home
    global outDoc, outSum, outTitles, ruledoutFile, titles, titlesDone, stopSections, stopSectionsFile

    outdir = options.lang + "-excl." + str_len_constraints

    print(" * Extracting Wikipedia in: {}".format(options.lang))
    exclFile = open(os.path.join(HOME, args.xwikisDir, outdir, "{}_titles_exclude.txt".format(options.lang)), 'r')

    # build the list of titles for the corresponding language
    titles = []
    for line in exclFile.readlines():
        titles.append(line.strip())

    print(" * # Titles for exclusion: {}".format(len(titles)))
    ...
    outDoc = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_documents.txt'.format(options.lang)), 'w')
    outSum = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_summaries.txt'.format(options.lang)), 'w')
    outTitles = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_titles.txt'.format(options.lang)), 'w')
    ruledoutFile = open(os.path.join(HOME, args.xwikisDir, outdir, '{}_titles_outoflen.txt'.format(options.lang)), 'w')
