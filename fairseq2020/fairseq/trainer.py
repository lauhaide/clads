...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            print(last_optim["criterion_name"])
            print(self.get_criterion().__class__.__name__)
            print(last_optim["optimizer_name"])
            print(self.optimizer.__class__.__name__)

            assert ((
                (last_optim["criterion_name"] == self.get_criterion().__class__.__name__)
                or
                (last_optim["criterion_name"] == "LabelSmoothedCrossEntropyCriterion"
                   and self.get_criterion().__class__.__name__ == "_CompositeLoss"))
            ...
            # is next line that kills with memory
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            #print(list(" ".join([self.task.tgt_dict.symbols[t] for t in s]) for s in
            #           sample['net_input']['prev_output_tokens'][:5].tolist()))
    ...
    def target_step(self, train_task, sample, raise_oom=False):
        with torch.no_grad():
            self.model.eval()

            try:
                targets = self.task.target_step(train_task, sample, self.model)
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.target_step(train_task, sample, raise_oom=True)
                raise e
        return targets
