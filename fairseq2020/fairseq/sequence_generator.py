...
class SequenceGenerator(nn.Module):
    ...
    def generate(self, models, sample: Dict[str, Dict[str, Tensor]], **kwargs):
        ...
        #print(list(" ".join([self.tgt_dict.symbols[t] for t in s]) for s in sample['net_input']['prev_output_tokens'][:1].tolist()))
        #print(list(" ".join([self.tgt_dict.symbols[t] for t in s]) for s in sample['target'][:1].tolist()))
        #print(list(" ".join([self.tgt_dict.symbols[t] for t in s]) for s in sample['net_input']['src_tokens'][:1].tolist()))
        #exit()
