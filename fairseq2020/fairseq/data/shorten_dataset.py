...
class TruncateDataset(BaseWrapperDataset):
    ...
    def sizes(self):
        ...
        return np.minimum(self.dataset.sizes, np.full((len(self.dataset.sizes)), self.truncation_length, dtype=int))
...
class FilteredDataset(BaseWrapperDataset):
    ...
    def __init__(self, dataset, truncation_length):
        ...
        szs = np.asarray(self.dataset.tgt_sizes[idxs])
        self.indices = list(idxs[np.where(szs <= truncation_length)])
        print("*    Filter pair on target: {}, remain:{}".format(len(dataset) - len(self.indices), len(self.indices)))
