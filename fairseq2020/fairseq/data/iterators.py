    ...
    def __iter__(self):
        for x in self.iterable:
            if self.n >= self.total:
                print("Iterator's end", self.n, self.total)
                return
            self.n += 1
            yield x
...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def __init__(
        ...
        self.buffer_size = min(buffer_size, 40)
...
class TaskGroupedIterator(CountingIterator):
    """Wrapper around an iterable that returns groups (chunks) of items.
    ...
    def __init__(self, iterable, chunk_size):
        itr = _taskchunk_iterator(iterable, chunk_size)
        super().__init__(
            itr,
            start=int(math.ceil(getattr(iterable, 'n', 0) / float(chunk_size))),
            total=int(math.ceil(len(iterable) / float(chunk_size))),
...
def _taskchunk_iterator(itr, chunk_size):
    chunk = {}
    for x in itr:
        if not x:
            print("Underline iterator's END:", getattr(itr, 'n', -1), getattr(itr, 'total', -1))
            break # at the end of the iterator this will come None... shouldnt?
        for k in x.keys():
            try:
                chunk[k].append(OrderedDict([(k, x[k])]))
            except:
                chunk[k] = []
                chunk[k].append(OrderedDict([(k, x[k])]))

        picktask = list(chunk.keys())[0]
        if len(chunk[picktask]) == chunk_size:
            yield chunk
            chunk = {}
    if len(chunk) > 0:
        yield chunk